...
public class PaymentService {
    ...
    public Transaction ProcessTransaction(Long sourceAccountId, Long destAccountId, BigDecimal amount){
        ...
        Account source = accountRepository.findById(sourceAccountId).orElseThrow(() -> new IllegalArgumentException("Melhorar erro"));
        Account destination = accountRepository.findById(destAccountId).orElseThrow(() -> new IllegalArgumentException("Melhorar erro"));

        Transaction transaction = new Transaction(id, sourceAccountId, destAccountId, amount, TransactionStatus.PENDING);
