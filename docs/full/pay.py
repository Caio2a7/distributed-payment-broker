...
STATUS_NAMES = {1: "PENDING", 2: "REJECTED", 3: "ACCEPTED", 4: "COMPLETED", 5: "FAILED"}


def send_udp(packet, req_id, gross_amount, mid):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(3.0)
    try:
        sock.sendto(packet, (GATEWAY_HOST, GATEWAY_UDP_PORT))
...
def send_tcp(packet, req_id, gross_amount, mid):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(3.0)
    try:
        sock.connect((GATEWAY_HOST, GATEWAY_TCP_PORT))
...
def send_http(mid, card, amount, installments, op, month, year, req_id):
    url = f"http://{GATEWAY_HOST}:{GATEWAY_HTTP_PORT}/payments"
    payload = {
        "mid": mid,
        "cardNumber": card,
        "amount": amount,
    ...
    try:
        with urllib.request.urlopen(req, timeout=3.0) as resp:
            ...
            print(
                f"[OK] Req #{req_id:02d} [HTTP] | Status: {status} | Bruto: R$ {amount / 100:.2f} | Líquido: R$ {liquid / 100:.2f} | Loja: {mid} | Hash: {fp}"
            )
...
def send_grpc(packet, req_id, gross_amount, mid):
    channel = grpc.insecure_channel(f"{GATEWAY_HOST}:{GATEWAY_GRPC_PORT}")
...
def print_binary_response(resp_data, proto_label, req_id, gross_amount, mid):
    ...
    if resp_type == 2:
        ...
        print(
            f"[OK] Req #{resp_req_id:02d} [{proto_label}] | Status: {status_str} | Bruto: R$ {gross_amount / 100:.2f} | Líquido: R$ {liquid_amount / 100:.2f} | Loja: {mid} | Hash: {fingerprint}"
        )
    elif resp_type == 99:
        ...
        print(
            f"[WARN] Req #{resp_req_id:02d} [{proto_label}] | Tipo inesperado: {resp_type}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Cliente de Pagamento Multi-Protocolo (UDP, TCP, HTTP, gRPC) para o DPB Gateway"
    )
    parser.add_argument(
        "-p",
        "--protocol",
        choices=["udp", "tcp", "http", "grpc"],
        default="udp",
        help="Protocolo a ser testado (udp, tcp, http, grpc)",
    )
    parser.add_argument(
        "--mid",
        default=None,
        help="MID do lojista (15 dígitos). Se omitido, sorteia aleatoriamente entre os cadastrados.",
    )
    parser.add_argument(
        "--card",
        default="4111111111111111",
        help="Número do cartão de crédito (Visa válido)",
    )
    parser.add_argument(
        "--amount",
        type=int,
        default=10000,
        help="Valor bruto em centavos (ex: 10000 = R$ 100.00)",
    )
    parser.add_argument(
        "--installments", type=int, default=1, help="Número de parcelas (1 a 12)"
    )
    parser.add_argument(
        "--op", choices=["PAYMENT", "REFUND"], default="PAYMENT", help="Operação"
    )
    ...
    parser.add_argument(
        "--req-id", type=int, default=1, help="ID sequencial da requisição"
    )
    ...
    try:
        if args.protocol == "udp":
            send_udp(packet, args.req_id, args.amount, mid)
        elif args.protocol == "tcp":
            send_tcp(packet, args.req_id, args.amount, mid)
        elif args.protocol == "grpc":
            send_grpc(packet, args.req_id, args.amount, mid)
    except socket.timeout:
        print(
            f"[TIMEOUT] Req #{args.req_id:02d} [{args.protocol.upper()}] | Sem resposta do Gateway"
        )
        sys.exit(1)
    except Exception as e:
