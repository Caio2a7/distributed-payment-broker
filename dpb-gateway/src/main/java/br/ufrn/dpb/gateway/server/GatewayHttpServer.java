...
public class GatewayHttpServer implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (running.get()) {
                ...
                workerPool.submit(() -> handleHttpClient(socket));
            }
    ...
    private void handleHttpClient(Socket socket) {
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream(), StandardCharsets.UTF_8));
            ...
            if (requestLine == null || requestLine.isBlank()) {
                sendHttpResponse(socket, out, 400, "{\"error\":\"Empty request\"}");
                return;
            }

            StringTokenizer tokenizer = new StringTokenizer(requestLine);
            if (!tokenizer.hasMoreTokens()) {
                sendHttpResponse(socket, out, 400, "{\"error\":\"Malformed request line\"}");
                return;
            }
            ...
            if (!httpMethod.equals("POST")) {
                sendHttpResponse(socket, out, 405, "{\"error\":\"Method Not Allowed - Only POST is supported for payment operations\"}");
                return;
            }
            ...
            if (contentLength <= 0) {
                sendHttpResponse(socket, out, 400, "{\"error\":\"Missing or empty body (Content-Length required)\"}");
                return;
            }
            ...
            if (mid == null || cardNumber == null || amountStr == null) {
                sendHttpResponse(socket, out, 400, "{\"error\":\"Required fields missing: mid, cardNumber, amount\"}");
                return;
            }
            ...
            if (authorizers.isEmpty()) {
                sendHttpResponse(socket, out, 503, "{\"error\":\"No authorizer instances available\"}");
                return;
            }
            ...
            try {
                chosenAuthorizer.getSocket().setSoTimeout(5000);
            ...
            if (response.getType() == Message.Type.RESPONSE) {
                ...
                sendHttpResponse(socket, out, 200, jsonResponse);
            } else {
                String errorMsg = new String(response.getPayload(), StandardCharsets.UTF_8);
                sendHttpResponse(socket, out, 400, "{\"error\":\"" + errorMsg.replace("\"", "\\\"") + "\"}");
            }

        } catch (Exception e) {
            try (DataOutputStream out = new DataOutputStream(socket.getOutputStream())) {
                sendHttpResponse(socket, out, 500, "{\"error\":\"" + e.getMessage() + "\"}");
    ...
    private void sendHttpResponse(Socket socket, DataOutputStream out, int statusCode, String jsonBody) throws IOException {
