...
public class GatewayTcpServer implements Runnable {
    ...
    private void handleConnection(Socket socket) {
        try {
            SingleSocketChannel channel = new SingleSocketChannel(socket, 0);
            ...
            while (channel.isConnected() && currentMsg != null) {
                if (currentMsg.getType() == Message.Type.LEDGER_REQUEST) {
                    ...
                    if (ledgers.isEmpty()) {
                        ...
                        SingleSocketChannel ledgerChannel = ledgers.get(ledgers.size() - 1);
                        try {
                            Message resp;
                            ledgerChannel.getLock().lock();
                            try {
                                ledgerChannel.getSocket().setSoTimeout(5000);
                                ...
                                resp = new Message(Message.Type.LEDGER_RESPONSE, currentMsg.getRequestId(), ledgerResp.getPayload());
                            } finally {
                                ledgerChannel.getLock().unlock();
                            }
